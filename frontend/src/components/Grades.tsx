...
export default function Grades() {
    ...
    const studentsWithGrades = students.filter(s => grades.some(g => g.studentId === s.id));
    const filteredStudents = studentsWithGrades.filter(s => {
        const q = search.toLowerCase();
        const studentGradesCourses = grades
            .filter(g => g.studentId === s.id)
            .map(g => cName(g.courseId))
            .join(" ");
        return `${s.firstName} ${s.lastName} ${s.groupName} ${studentGradesCourses}`.toLowerCase().includes(q);
    });

    const allFilteredGrades = grades.filter(g => filteredStudents.some(s => s.id === g.studentId));
    const avg = allFilteredGrades.length
        ? (allFilteredGrades.reduce((s, g) => s + g.value, 0) / allFilteredGrades.length).toFixed(1)
        : null;
        ...
        <div>
            ...
            <div className="flex items-baseline justify-between mb-6">
                <div className="flex items-baseline gap-3">
                    ...
                    <span className="text-sm text-zinc-400">{filteredStudents.length} students</span>
                ...
                <div className="flex gap-2">
                    {isTeacher && (
                        <Btn
                            variant="ghost"
                            onClick={() => exportCsv(allFilteredGrades, sName, cName)}
                            disabled={allFilteredGrades.length === 0}
                        >
                ...
                <Table>
                    ...
                    {filteredStudents.length === 0
                        ...
                        : filteredStudents.map(s => {
                            ...
                            const avgVal = sg.length
                                ? sg.reduce((acc, g) => acc + g.value, 0) / sg.length
                                : null;
                            const courseList = [...new Set(sg.map(g => g.courseId))]
                                .map(cid => cName(cid))
                                .join(", ");
                            return (
                                <Tr
                                ...
                                >
                                    ...
                                    <Td className="text-zinc-500">{s.groupName}</Td>
                                    <Td className="text-zinc-500 max-w-xs truncate">{courseList}</Td>
                                    <Td>
                                        {avgVal !== null
                                            ? <Badge text={avgVal.toFixed(1)} color={gradeColor(Math.round(avgVal))} />
                                            : "—"}
                                    </Td>
                                    <Td className="text-zinc-400">{sg.length}</Td>
                                </Tr>
                            );
                        })
                    }
